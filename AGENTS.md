...
| `REMINDER_EMAIL_SENDER` | Gmail address to send reminders from |
| `REMINDER_EMAIL_RECEIVERS` | Comma-separated recipient emails |
| `REMINDER_EMAIL_PASSWORD` | Gmail App Password for SMTP fallback only |
| `REMINDER_TIMEZONE` | Reminder timezone, defaults to `Asia/Karachi` |
| `REMINDER_WEEKDAY` | Reminder weekday, defaults to `4` (Friday, Monday=0) |
| `REMINDER_HOUR` | Reminder hour, defaults to `18` |
| `REMINDER_MINUTE` | Reminder minute, defaults to `0` |
| `CAREERPILOT_ENABLE_EMAIL_SCHEDULER` | Set to `false` to disable the FastAPI startup scheduler |

---


## Weekly Email Reminder (Friday 6PM PKT)

CareerPilot sends the weekly progress email every Friday at 6PM PKT. The primary sender is Gmail API (`scripts/send_gmail_api.py`). SMTP in `scripts/weekly_reminder.py` is only a fallback when a Gmail app password is configured.
...
3. Optional: set `REMINDER_TIMEZONE=Asia/Karachi` in `config/.env`
4. Test with: `python scripts/send_gmail_api.py`
5. Keep one scheduler alive:
   - local cron job
   - FastAPI process (`api/server.py` starts `scripts/reminder_scheduler.py`)
   - standalone daemon (`python scripts/careerpilot_daemon.py`)

Recommended cron format:
...
The currently installed local cron job uses:
...
If the email does not arrive, check `logs/weekly_email.log`, verify `credentials/token.json` is refreshable, and confirm the local machine or deployed process was running at Friday 6PM PKT.
